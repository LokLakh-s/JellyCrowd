...
- ☑ **N40b — Liens header optionnels (Discord + soutien).** Deux icônes admin-activables à **gauche** du header, juste après le logo, centrées en hauteur (`insertHeaderLinks`, idempotent) : **Discord** (lien d'invitation) puis **Soutien** (pièces → page de don). Off par défaut ; config `DiscordInviteEnabled`/`DiscordInviteUrl` + `SupportLinkEnabled`/`SupportLinkUrl` (page admin), exposées via `Settings/Language` **uniquement si activées**. SVG inline (CSP-safe).
