...
- ☑ **Bug épisodes faussement « Disponible ».** Demander une saison (HotD S3, seul l'ép. 1 sorti) marquait **les 10 épisodes Disponibles** dès que la série existait dans Jellyfin : le réconciliateur matchait la **série** par TMDB id, pas l'**épisode**. Corrigé — `ILibraryMatcher.FindEpisodeItemId(seriesTmdbId, season, episode)` (filtre `ParentIndexNumber`/`IndexNumber` sous la série) ; `RequestReconciler` l'utilise pour toute requête tv avec saison/épisode. 2 tests de non-régression.
- ☑ **Bug notifs e-mail à l'admin pour les demandes on-behalf.** La **boîte ops** (`NotificationEmailTo`) recevait **tous** les événements de **tous** les utilisateurs (d'où 10 mails pour HotD), alors que les notifs d'une requête appartiennent à son **propriétaire** (déjà livrées sur ses canaux perso). Corrigé — gating par événement comme Discord (`EmailNotify{Created,Approved,Denied,Available}`), défauts : **Created on**, le reste **off** ; toggles dans l'onglet *Settings*. *(Couvre une partie de N15 côté bruit de notifs ; l'agrégation reste à faire.)*
- ☑ **N15 — Agrégation des demandes de saison en cours.** Dans *Mes demandes*, les requêtes par épisode d'une même saison (TmdbId+Season) sont **regroupées en une seule ligne** « … · Saison N » : compteur **{dispo}/{total} épisodes**, **date du prochain épisode** (date de sortie future la plus proche parmi les épisodes manquants), badge de statut agrégé, **Cancel / Retry groupés** (sur tous les épisodes), et badges de progression DL gérés via `data-req-ids`. Le suivi par épisode reste **backend**. Le flot de fausses notifs *Available* était déjà réglé (matching épisode-exact, lot 2). *(Une seule notif « saison dispo » = amélioration future possible.)*
- ☑ **N16 — Titres cliquables dans le menu « Demander la saga ».** Chaque film du récap de saga est **cliquable** (`jellycrowd-link` → `openModal(p)`) et ouvre son propre popup.
- ☑ **N17 — Section « Related media » en bas du popup média.** Bande **Related media** en bas du popup : recommandations TMDB (`GET Catalog/Related/{type}/{id}`, dispo enrichie via `ExecuteAsync`), **vignettes cliquables** (`renderCard`) qui ouvrent leur propre popup en fermant l'actuel. Scroll horizontal.
- ☑ **N18 — Purge vérifiée + retry (intégrité de suppression).** `IDownloadClient.PurgeAsync`/`IDownloadDispatcher.PurgeAsync` renvoient désormais un **succès/échec**. `DeletionTask` ne **finalise** la suppression (delete Jellyfin + retrait de la requête) **qu'une fois la purge backend confirmée** ; sinon (backend down) la requête **reste flaggée et est réessayée** au prochain passage horaire. Garde-fou : au-delà de `rétention + 7 j`, suppression quand même (un backend retiré ne bloque jamais éternellement). Test ajouté.
- ☑ **N19 — Suppression en cascade multi-backends.** *(Livré — voir N19/N30 plus haut : purge Radarr/Sonarr + fichiers + file RDT via `PurgeAsync`.)*
...
- ☑ **N22 — Bandeau annonce multi-lignes.** Le texte d'annonce **wrappe sur 3 lignes max** (clamp) au lieu d'une seule ligne tronquée, dans la limite de `40vw` (et déjà sur sa propre ligne pleine largeur en mobile). Plus de troncature agressive.
- 🚫 **N23 — Accès config dans la section « Server ».** **Non faisable proprement** via l'API plugin. La page est déjà `EnableInMainMenu = true` + `MenuIcon` (`Plugin.cs`) → elle apparaît donc **dans le menu principal du dashboard** (pas seulement sous *My Plugins*). `PluginPageInfo` (Jellyfin 10.11) n'expose **aucun contrôle de section/ordre** : forcer la rubrique « Server » en haut exigerait une injection DOM dans le dashboard (fragile, comme le header) — écarté. *(État optimal atteint avec l'API dispo.)*
- ☑ **N24 — Quota visible par utilisateur (admin).** L'onglet *User quotas* affiche une colonne **Usage** (utilisé / quota en Gio, + palier adaptatif) par utilisateur, via le nouvel endpoint admin `GET JellyCrowd/Quota/All` (`UserQuotaInfoDto`).
- ☑ **N25 — Bouton Retry par requête (admin).** Bouton **Retry** sur chaque requête `Approved` de l'onglet *Requests* admin → `POST {id}/Retry` (relance la recherche backend). Le endpoint Retry autorise désormais l'**admin à relancer n'importe quelle requête** (y compris on-behalf ; avant : 404 si pas propriétaire).
- ☑ **N26 — Auto-retry des dispatches bloqués.** `IDownloadDispatcher.RetryStuckAsync` (appelé par `DownloadDispatchTask` toutes les 5 min) relance automatiquement la recherche des requêtes `Approved` déjà dispatchées mais jamais devenues `Available` (indexeurs morts, grab échoué) — back-off **6 h**, abandon après **14 j** (relance manuelle au-delà). Plus besoin de relancer à la main. 3 tests.
- ☑ **N27 — Requête multi-saisons : monitorer TOUTES les saisons.** Quand la série est déjà dans Sonarr, le dispatch **monitore la saison demandée** (mute le tableau `seasons`, `PUT /series/{id}` via `UpdateSeriesAsync`) **avant** `SeasonSearch` — sinon les saisons ajoutées après la 1ʳᵉ restaient non-monitorées et ne se téléchargeaient pas (cas Hannibal). 2 tests.
- ☑ **N28 — Quota théorique à la demande.** ~~`GetUsageAsync` compte les requêtes **en vol** (`Pending`/`Approved`) à l'estimation… en plus de la taille réelle des `Available` ; la barre reflète le provisoire immédiatement.~~ **Annulé par N38** : l'affichage ne compte plus que les `Available` à leur taille réelle.
- ☑ **N29 — *Ma bibliothèque* arborescente (série → saison → épisode).** Les séries s'affichent en **arbre dépliable** (caret ▶/▼) : série › saisons › épisodes, **bouton supprimer à chaque niveau** (série entière / saison / épisode — flag toutes les requêtes couvertes, avec *Keep* + compte à rebours si déjà flaggé). Les films restent en lignes simples. DTO `MediaUsageDto` enrichi de `Episode`.
- ☑ **N19 / N30 — Cascade de suppression complète.** Quand `DeletionTask` supprime un média qui n'appartient **plus à personne**, il appelle d'abord `IDownloadDispatcher.PurgeAsync` → `IDownloadClient.PurgeAsync` : **Radarr** (delete movie + fichiers + sortie de file RDT) ou **Sonarr** (delete **série entière** + fichiers + file via `DeleteSeriesAsync`/`ParseSeriesQueueRecordIds`), puis suppression Jellyfin/disque (`MediaDeleter`). Une re-demande repart donc propre. `CancelAsync` (annulation user d'une saison) ne supprime **toujours pas** la série entière. *(Prowlarr ne stocke aucun média par titre → rien à purger ; RDT couvert par `removeFromClient`.)* 3 tests.
- ☑ **N31 — Unreleased multi-demandeurs : appartenance à tous.** Le réconciliateur promeut désormais en `Available` **toute** requête active sur un titre présent en bibliothèque — `Approved` **et** `Pending` (avant : `Approved` seulement, donc un co-demandeur encore en attente d'approbation n'héritait jamais de l'appartenance). Tout le monde possède le média à la mise à dispo, sans re-téléchargement (propriété partagée). Test ajouté.
...
- ☑ **N33 — Fond ramené sur Home à l'ouverture d'un volet.** À chaque **ouverture fraîche** d'un volet, la page derrière l'overlay est envoyée sur **Home** (`sendBackgroundHome`, via le bouton home natif), avec un drapeau `suppressHashClose` (timer 200 ms) pour que cette navigation ne ferme pas notre overlay. Résultat : fermer le volet (×, Échap, re-clic, nav) ramène sur Home.
- ☑ **N34 — Popup série après disponibilité partielle.** Une série utilise **toujours** le sélecteur de saisons (pleine largeur), même quand elle est déjà partiellement en bibliothèque : on liste toutes les saisons (les manquantes restent demandables), + un lien **Ouvrir dans Jellyfin** si dispo. Seuls les **films** disponibles gardent le flux « Add to my library ». (Avant : `item.Available` au niveau série basculait sur le flux film → impossible de demander les autres saisons.)
- ☑ **N35 — Casting trié + Réalisateur & Scénariste cliquables.** Le casting est trié par **ordre de générique** TMDB (`order`, comme IMDb) ; la ligne crédits affiche **Réalisateur** *et* **Scénariste** (`Writers` parsé depuis `credits.crew` — job/department Writing/Screenplay/Story), tous **cliquables** → filmographie. Modèle `CatalogItem.Writers` ; 2 tests.
- ☑ **N36 — Alignement vertical navbar.** La cloche de notif et le bandeau d'annonce reçoivent `align-self:center` (comme la barre de quota) pour être centrés verticalement dans la barre comme les liens nav (ils paraissaient un poil trop bas). *(À reverifier en live ; capture si l'écart persiste.)*
...
- ☑ **N40a — Titre de la critique stocké à la création.** `MediaComment.Title` (+ `CommentDto.Title`) capturé au POST (catalog.js + reviews de la fiche native), backfillé sur ré-édition ; la page Modération affiche le **vrai titre** (repli « Film/Série #id » pour les anciennes critiques sans titre).
- ☑ **N40b — Liens header optionnels (Discord + soutien).** Deux icônes admin-activables à **gauche** du header, juste après le logo, centrées en hauteur (`insertHeaderLinks`, idempotent) : **Discord** (lien d'invitation) puis **Soutien** (pièces → page de don). Off par défaut ; config `DiscordInviteEnabled`/`DiscordInviteUrl` + `SupportLinkEnabled`/`SupportLinkUrl` (page admin), exposées via `Settings/Language` **uniquement si activées**. SVG inline (CSP-safe). **Style (retour live)** : boutons natifs `paper-icon-button-light headerButton`, **verts** (#4caf50) et plus grands (24px) comme les boutons de droite, groupés juste après le logo.
- ☑ **N40c — Onglet « Appartenance » (admin).** Onglet admin (via `ensureAdminTabs`, à côté de Modération) → `ownership.html`/`ownership.js` : qui possède quel média. `GET JellyCrowd/Requests/Ownerships` (admin) regroupe les requêtes `Available` par scope exact (film / saison / épisode) avec la liste des propriétaires (nom + depuis quand) ; affiche poster, badge de scope (S3/S2E5), compteur, filtre client (titre/utilisateur). DTO `MediaOwnershipDto`/`OwnerDto`. 1 test.
