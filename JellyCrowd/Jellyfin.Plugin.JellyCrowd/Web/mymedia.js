...
(function () {
  ...
  // Confirmation in our own dialog (no window.confirm): resolves true only on confirm; Escape, cancel and
  // a click outside mean no. Focus is trapped in the dialog and handed back to the trigger afterwards.
  ...
  function historyRow(entry) {
    ...
    // Remove a single entry (no confirm — it is one line, and clearing everything is the guarded action).
    var del = document.createElement('button');
    del.type = 'button';
  ...
  function renderHistory(data) {
    var section = document.getElementById('jcHistorySection');
    var listEl = document.getElementById('jcHistoryList');
    var toggle = document.getElementById('jcHistoryToggle');
    var clear = document.getElementById('jcHistoryClear');
    var msg = document.getElementById('jcHistoryMessage');
    section.hidden = false;
    document.getElementById('jcHistoryTitle').textContent = t('history_title');
    listEl.innerHTML = '';
    msg.hidden = true;

    var hidden = data && data.Hidden;
    // The toggle turns the personal history off/on; recording continues either way.
    ...
    if (hidden) {
      clear.hidden = true;
      msg.textContent = t('history_hidden');
      msg.hidden = false;
      return;
    }
    ...
    if (!entries.length) {
      clear.hidden = true;
      msg.textContent = t('history_empty');
      msg.hidden = false;
      return;
    }
    ...
    clear.onclick = function () {
      confirmAction({ title: t('history_clear'), message: t('history_clear_confirm'), confirmLabel: t('history_clear') })
        .then(function (ok) {
          if (!ok) { return; }
          clear.disabled = true;
          apiPost('JellyCrowd/History/Mine/Clear')
            .then(loadHistory)
            .catch(function () { clear.disabled = false; });
        });
    };

    entries.forEach(function (e) { listEl.appendChild(historyRow(e)); });
  }
  ...
  function loadHistory() {
    ...
    return apiGet('JellyCrowd/History/Mine')
      .then(renderHistory)
      .catch(function () { /* history is best-effort; leave the section as-is */ });
  }
